...
HRESULT DxgiSpoofing::hkGetDesc3(IDXGIAdapter4* This, DXGI_ADAPTER_DESC3* pDesc)
{
    ...
    // AMD FFX DLLs must always see the real GPU info to register FSR4 providers correctly
    if (iequals(caller, "vulkan-1.dll") || iequals(caller, "amdvlk64.dll") || iequals(caller, "dxgi.dll") ||
        ...
        iequals(caller, "amdxcffx64.dll") || iequals(caller, "amd_fidelityfx_upscaler_dx12.dll"))
    {
        return result;
    }
