...
void Amdxc64Hooks::Init()
{
    ...
    {
        // amdxc64.dll initialization loads amdxcffx64.dll, which caches adapter info;
        // skip spoofing so both DLLs see real AMD GPU from the start.
...
HRESULT STDMETHODCALLTYPE Amdxc64Hooks::hkAmdExtD3DCreateInterface(IUnknown* pOuter, REFIID riid, void** ppvObject)
{
    ...
    else if (runFsr4Upgrade && riid == __uuidof(IAmdExtFfxApi))
    {
        ...
        *ppvObject = amdExtFfxApi;
        ...
        if (o_AmdExtD3DCreateInterface != nullptr && amdExtFfxApi->o_amdExtFfxApi == nullptr)
        {
            auto hr = o_AmdExtD3DCreateInterface(pOuter, riid, (void**)&amdExtFfxApi->o_amdExtFfxApi);
            LOG_INFO("Obtained real IAmdExtFfxApi from driver, hr:{:#x} ptr:{}", hr, (void*)amdExtFfxApi->o_amdExtFfxApi);
        }
