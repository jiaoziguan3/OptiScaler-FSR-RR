...
HRESULT STDMETHODCALLTYPE AmdExtFfxApi::UpdateFfxApiProvider(void* pData, uint32_t dataSizeInBytes)
{
    ...
    if (o_UpdateFfxApiProvider == nullptr)
    {
        ...
        // amdxcffx64.dll caches adapter info during DLL_PROCESS_ATTACH;
        // skip spoofing for the entire load so it sees real AMD GPU from the start.
        ScopedSkipSpoofing skipSpoofDuringLoad {};
        ...
        {
            const auto& customPath = Config::Instance()->Fsr4Amdxcffx64Path.value();
        ...
        effectType == FFXStructType::SwapchainDX12)
    {
        ...
        // amdxcffx64.dll queries the DXGI adapter description internally;
        // skip spoofing so it sees real AMD GPU info and doesn't reject the call.
        ScopedSkipSpoofing skipSpoof {};

        // Prefer the real driver vtable (has D3D12 device context bound) over raw exports
        if (o_amdExtFfxApi != nullptr)
        {
            LOG_INFO("UpdateFfxApiProvider via driver vtable (device-bound, spoof skipped), effect:{}", effect);
            result = o_amdExtFfxApi->UpdateFfxApiProvider(pData, dataSizeInBytes);
        }
        else if (o_UpdateFfxApiProviderEx != nullptr)
        {
            magicData data = { { 0, 1, 1, 0 }, nullptr };
            result = o_UpdateFfxApiProviderEx(pData, dataSizeInBytes, &data);
        }
        else if (o_UpdateFfxApiProvider != nullptr)
        {
            result = o_UpdateFfxApiProvider(pData, dataSizeInBytes);
        }
