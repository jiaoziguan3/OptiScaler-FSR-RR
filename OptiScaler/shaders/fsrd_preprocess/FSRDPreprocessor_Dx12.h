...
class FSRDPreprocessor_Dx12
{
  public:
    ...
    enum class ConvFlags : uint32_t
    {
        ...
        JitteredMV =            1 << 4, // Input motion vectors include jitter offsets; shader will cancel them
    ...
    struct ConversionDesc
    {
        ...
        DirectX::XMFLOAT2 JitterCorrection; // (currJitter - prevJitter) / MVScale, used when input MVs contain jitter
  ...
  public:
     ...
     * @note v1.2.0: Mode-1 composited radiance is mapped to indirect diffuse signal.
     */
    void GetSignal(ffxDispatchDescDenoiserIndirectDiffuse& signalDesc, ffxDispatchDescDenoiser& dispatchDesc) const;

    /**
     * @brief Configures input/output resources after input conversion for FSR-RR with Mode-2 discrete diffuse/specular color.
     * Resources are transitioned to SRV state and valid until the next conversion or composition dispatch.
     * Must be re-acquired after each dispatch (lifetime managed internally).
     * @note v1.2.0: Uses separate indirect specular and indirect diffuse signal descriptors linked via pNext.
     */
    void GetSignal(ffxDispatchDescDenoiserIndirectSpecular& specularSignalDesc, ffxDispatchDescDenoiserIndirectDiffuse& diffuseSignalDesc, ffxDispatchDescDenoiser& dispatchDesc) const;
    ...
    /**
     * @brief Optional post-composition EMA temporal stabilization layer.
     * Blends the composition output with the previous frame's stabilized output to suppress
     * residual per-frame flicker. No reprojection - a disocclusion guard avoids ghosts.
     * When enabled, GetCompositionOutput() returns the stabilized result.
     * @param alpha Contribution of the current frame [0..1]. 0.5 = balanced.
     * @param reset Bypass blend on first frame / camera cut.
     */
    bool DispatchTemporalStable(ID3D12GraphicsCommandList* cmdList, float alpha, bool reset);

    /**
     * @brief Enables or disables the temporal stabilization layer.
     * When disabled, GetCompositionOutput() returns the raw composition output.
     */
    void SetTemporalStableEnabled(bool enabled);

    /**
     * @brief Copies the contents of the given source texture.
     * @note srcTex must be in a shader-readable state (SRV). dstTex is transitioned
     *       COMMON -> UAV -> COMMON internally.
     */
