...
void FSR31FeatureDx12::ConfigureUpscalerResponse()
{
    const auto& cfg = *Config::Instance();
    ApplyUpscalerResponse(cfg.FsrShadingScale.value_or_default(), cfg.FsrAccAddPerFrame.value_or_default());
}

void FSR31FeatureDx12::ApplyUpscalerResponse(float shadingScale, float accumulationAddedPerFrame)
{
    const auto& cfg = *Config::Instance();
    CustomOptional<float> effectiveShadingScale { shadingScale };
    CustomOptional<float> effectiveAccumulation { accumulationAddedPerFrame };
    ...
    SetFfxUpscaleKeyValue(&_context, _shadingScale, effectiveShadingScale,
                          FFX_API_CONFIGURE_UPSCALE_KEY_FSHADINGCHANGESCALE, "Shading Scale");
    SetFfxUpscaleKeyValue(&_context, _accAddPerFrame, effectiveAccumulation,
...
bool FSR31FeatureDx12::CreateUpscalerContext(const NVSDK_NGX_Parameter& ngxParams) 
{
...

    {
        ...
        {
            LOG_ERROR("_context error: {0}", FfxApiProxy::ReturnCodeToString(ret));
            return false;
        }
    }
...
void FSR31FeatureDx12::ConfigureUpscaler(const NVSDK_NGX_Parameter& inParams, ffxDispatchDescUpscale& upscalerDesc)
{
    ...
    {
        SetFfxUpscaleKeyValue(&_context, _velocity, cfg.FsrVelocity, FFX_API_CONFIGURE_UPSCALE_KEY_FVELOCITYFACTOR,
                              "Velocity");
    }

    // Reactiveness, Shading, and Accumulation (FSR 3.1.4+)
    if (Version() >= feature_version { 3, 1, 4 })
        ConfigureUpscalerResponse();
