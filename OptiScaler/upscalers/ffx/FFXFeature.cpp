...
{
    LOG_DEBUG("QueryVersionsDx12 called, device:{}", (void*)device);
    ...
    auto result1 = FfxApiProxy::D3D12_Query(nullptr, &versionQuery.header);
    
    LOG_DEBUG("QueryVersionsDx12 first query result:{:#x} versionCount:{}", result1, versionCount);
    ...
    auto result2 = FfxApiProxy::D3D12_Query(nullptr, &versionQuery.header);
    
    LOG_DEBUG("QueryVersionsDx12 second query result:{:#x}", result2);
    for (uint64_t i = 0; i < versionCount; ++i)
    {
        LOG_INFO("QueryVersionsDx12 version[{}]: id:{:#x} name:{}", i, 
                 State::Instance().ffxUpscalerVersionIds[i],
                 State::Instance().ffxUpscalerVersionNames[i] ? State::Instance().ffxUpscalerVersionNames[i] : "(null)");
    }
}
