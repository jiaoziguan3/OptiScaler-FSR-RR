...
#pragma region debug-point fsrfg-antilag2-private-data-sample
            const auto setPrivateDataResult =
                pSwapChain->SetPrivateData(IID_IFfxAntiLag2Data, sizeof(antilag2_data), &antilag2_data);
            static std::atomic_uint64_t setPrivateDataCount = 0;
            const auto count = ++setPrivateDataCount;
            if (count % 120 == 0 || FAILED(setPrivateDataResult))
            {
                LOG_INFO("FSRFG AntiLag2 private data sample count:{} fg_state:{} frame_interpolated:{} swapchain:{:X} "
                         "context:{:X} mode:{} enabled:{} thread_id:{} result:{:#x}",
                         count, fg_state, frame_interpolated, (size_t) pSwapChain, (size_t) antilag2_data.context,
                         static_cast<int>(_lowLatencyMode), antilag2_data.enabled, GetCurrentThreadId(),
                         setPrivateDataResult);
            }
#pragma endregion debug-point fsrfg-antilag2-private-data-sample
...
#pragma region debug-point fsrfg-antilag2-private-data-clear
        const auto clearPrivateDataResult = pSwapChain->SetPrivateData(IID_IFfxAntiLag2Data, 0, nullptr);
        LOG_INFO("FSRFG AntiLag2 private data cleared swapchain:{:X} result:{:#x} thread_id:{}", (size_t) pSwapChain,
                 clearPrivateDataResult, GetCurrentThreadId());
#pragma endregion debug-point fsrfg-antilag2-private-data-clear
    }
}
