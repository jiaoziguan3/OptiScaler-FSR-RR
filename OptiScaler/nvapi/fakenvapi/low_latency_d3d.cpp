...
bool LowLatency::update_low_latency_tech(IUnknown* pDevice)
{
    ...
    const bool fsrfgLatencyFlexFallback = forced_low_latency_tech == LowLatencyMode::None &&
                                          State::Instance().activeFgOutput == FGOutput::FSRFG &&
                                          IdentifyGpu::getPrimaryGpu().vendorId != VendorId::Nvidia;
    const bool force_latencyflex = Config::Instance()->FN_ForceLatencyFlex.value_or_default() || fsrfgLatencyFlexFallback;
    ...
    if (!currently_active_tech.load())
    {
        ...
        if (!force_latencyflex && State::Instance().dlssgDetectedInterpolationCount <= 1 &&
    ...
    static bool last_force_latencyflex = force_latencyflex;
    static bool last_fsrfg_latencyflex_fallback = fsrfgLatencyFlexFallback;
    bool change_detected = last_force_latencyflex != force_latencyflex ||
                           last_fsrfg_latencyflex_fallback != fsrfgLatencyFlexFallback;
